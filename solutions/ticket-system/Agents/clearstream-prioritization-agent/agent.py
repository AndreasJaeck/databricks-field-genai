...
# Custom output parser for clearstream priorities
class JSONParser:
    ...
    def validate_priority_response(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate that the parsed JSON has the expected priority fields"""
        expected_fields = {
            "priority",
            "priority_reasoning"
        }
        ...
        valid_priorities = {"Immediate", "Urgent", "Normal", "Low"}
        if data["priority"] not in valid_priorities:
            return {"error": f"Invalid priority value. Must be one of: {valid_priorities}"}
    ...
    def __call__(self, input_data: Any) -> Dict[str, Any]:
        ...
        if "error" not in parsed:
            return self.validate_priority_response(parsed)
        return parsed
